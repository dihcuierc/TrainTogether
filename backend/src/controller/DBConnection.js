...
function collectOneRev(id){
    ...
    const reviewRef = collection(db, 'Review')
    const review = query(reviewRef, where("UID", "==", id))
...
async function addRevData(new_UID, new_comment, new_name, new_rating, new_exID){
    // add Review data
    const reviewRef = collection(db, "Review");
...
function collectOneGoal(id){
    ...
    const goalRef = collection(db, 'Goal')
    const goal = query(goalRef, where("UID", "==", id))
...
async function addGoalData(new_UID, new_Title, new_CV, new_TV, new_image, new_Deadline, new_Done){
    // add Goal data
    const goalRef = collection(db, "Goal");
    const UserRef = doc(db, "User", new_UID)
    const data = {
...
function deleteGoalData(UID, GID){
    // delete Goal data
    const goalRef = collection(db, "Goal")
    const goal = query(goalRef, where("UID", "==", UID), where("GID", "==", GID))

    getDocs(goal).then(function(snapshot) {
...
function collectCH(){
    const CHref = collection(db, "Calories History")
}

function collectOneCH(){
